...
```{r plot horses, fig.width=10, fig.height=3, fig.cap='Figure 1. Horse population between 1906 and 1972 in each Canadian province.', out.width="100%"}
...
max_horses <- horse_pop %>% 
  ...
  summarize(Maximum = max(Value),
            Minimum = min(Value))
kable(max_horses, caption = "Table 1. Minimum and maximum number of horses for each province from 1940 to 1972.")
```

Below we zoom in and look at the province of `r params$province`:

```{r plot province, fig.width=5, fig.height=4, fig.cap='Figure 2. Historical number of horses in `r params$province` between 1906 and 1972.'}
...
The highest number of horses in `r params$province` at a given time was `r format(horse_pop |> filter(GEO == params$province) |> arrange(desc(Value)) |> slice(1) |> pull(Value), big.mark=",")`.
